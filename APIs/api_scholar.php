...
switch ($method) {
    ...
    case 'PUT':
        $data = json_decode(file_get_contents("php://input"));
        $stmt = $pdo->prepare("UPDATE recipient SET name = ?, email = ?, school = ?, contact = ?, branch = ?, GradeLevel = ?, admission_date = ?, password = ? WHERE id = ?");
    ...
    case 'DELETE':
        if (isset($_GET['id'])) {
            $id = intval($_GET['id']);
            $stmt = $pdo->prepare("DELETE FROM recipient WHERE id = ?");
