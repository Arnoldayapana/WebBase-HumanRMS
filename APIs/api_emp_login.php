...
switch ($method) {
    case 'GET':
        if (isset($_GET['id'])) {
            $id = intval($_GET['id']);
            $stmt = $pdo->prepare("SELECT * FROM employee_login WHERE id = ?");
            $stmt->execute([$id]);
            ...
            $stmt = $pdo->query("SELECT * FROM employee_login");
    ...
    case 'POST':
        ...
        $stmt = $pdo->prepare("INSERT INTO employee_login (username, password) VALUES (?, ?)");
    ...
    case 'PUT':
        $data = json_decode(file_get_contents("php://input"));
        $stmt = $pdo->prepare("UPDATE employee_login SET username = ?, password = ? WHERE id = ?");
        $result = $stmt->execute([$data->username, $data->password, $data->id]);
    ...
    case 'DELETE':
        if (isset($_GET['id'])) {
            $id = intval($_GET['id']);
            $stmt = $pdo->prepare("DELETE FROM employee_login WHERE id = ?");
